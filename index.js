  ...
  async fetch(request, env) {
    ...
    if (url.pathname === '/api/form' && request.method === 'GET') {
      ...
      const response = {
        ...
        lastUpdated: userData ? JSON.parse(userData).lastUpdated : null // 返回最后更新时间
      };
    ...
    if (url.pathname === '/api/save' && request.method === 'POST') {
      ...
      const userData = JSON.parse(storedUser);
      userData.info = data;
      userData.lastUpdated = new Date().toISOString(); // 记录提交时间
      await kv.put(`user:${username}`, JSON.stringify(userData));

      // 发送 Telegram 通知
      const TELEGRAM_BOT_TOKEN = env.TELEGRAM_BOT_TOKEN; // 在环境变量中设置
      const TELEGRAM_CHAT_ID = env.TELEGRAM_CHAT_ID; // 在环境变量中设置
